...
export default function Main() {
  ...
  while (trueCount <= 3) {
    ...
    if (sign === '+') { logic = `${(mamba1 * 1) + (mamba2 * 1)}`; } else if (sign === '*') { logic = `${mamba1 * mamba2}`; } else { logic = `${mamba1 - mamba2}`; }

    const name = readlineSync.question('Your answer: ');

    if (name === logic) { bigDeal = 'Correct!'; } else { return console.log(`'${name}' is wrong answer ;(. Correct answer was '${logic}'.\nLet's try again, ${timmy}!`); }
